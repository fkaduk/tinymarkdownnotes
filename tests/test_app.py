...
class TestNoteCreation:
    ...
    def test_create_note_without_key_returns_404(self, client):
    ...
    def test_create_note_with_wrong_key_returns_404(self, client):
        """Creating note with wrong admin key returns 404."""
        response = client.get('/notes/test?key=wrong-key')
        assert response.status_code == 404

    def test_create_note_with_correct_key_succeeds(self, client):
        """Creating note with correct admin key succeeds."""
        response = client.get('/notes/test?key=test-admin-key')
        assert response.status_code == 200
        assert b'Note: test' in response.data
        assert b'First item' in response.data

    def test_invalid_slug_blocked(self, client):
        ...
        """
        response = client.get('/notes/../etc/passwd?key=test-admin-key')
    ...
    def test_slug_too_long_returns_400(self, client):
        ...
        long_slug = 'a' * 65
        response = client.get(f'/notes/{long_slug}?key=test-admin-key')
...
class TestNoteViewing:
    ...
    def test_view_existing_note(self, client):
        """Viewing an existing note works."""
        # Create note
        client.get('/notes/mytest?key=test-admin-key')

        # View note
        response = client.get('/notes/mytest')
        assert response.status_code == 200
        assert b'Note: mytest' in response.data
        assert b'First item' in response.data

    def test_view_note_includes_share_link(self, client):
        """Note page includes share link."""
        client.get('/notes/share-test?key=test-admin-key')
        response = client.get('/notes/share-test')

        assert response.status_code == 200
        assert b'Share this link' in response.data

    def test_note_has_preview_and_edit_tabs(self, client):
        """Note page has Preview and Edit tabs."""
        client.get('/notes/ui-test?key=test-admin-key')
        response = client.get('/notes/ui-test')
...
class TestNoteEditing:
    ...
    def test_edit_note_with_correct_version(self, client):
        """Editing note with correct version succeeds."""
        # Create note
        client.get('/notes/edit-test?key=test-admin-key')

        # Edit note
        response = client.post('/notes/edit-test', data={
            'markdown': '# Updated\n\n- [x] Done',
            'version': '1'
        ...
        # Verify edit
        response = client.get('/notes/edit-test')
        assert b'Updated' in response.data
        assert b'Done' in response.data

    def test_edit_note_with_wrong_version_returns_409(self, client):
        """Editing note with wrong version returns conflict."""
        # Create note
        client.get('/notes/conflict-test?key=test-admin-key')

        # Try to edit with wrong version
        response = client.post('/notes/conflict-test', data={
    ...
    def test_edit_note_increments_version(self, client):
        """Editing note increments version number."""
        from pathlib import Path
        import app as app_module

        # Create note
        client.get('/notes/version-test?key=test-admin-key')

        # Edit note
        client.post('/notes/version-test', data={
            'markdown': '# Version 2',
            'version': '1'
        })
    ...
    def test_edit_nonexistent_note_returns_404(self, client):
        ...
        response = client.post('/notes/doesnotexist', data={
            'markdown': '# Test',
            'version': '1'
        })
        assert response.status_code == 404

    def test_edit_note_too_large_returns_413(self, client):
        """Editing note with content too large returns 413."""
        # Create note
        client.get('/notes/large-test?key=test-admin-key')

        # Try to save huge content
        large_content = 'x' * 200_000
        response = client.post('/notes/large-test', data={
...
class TestSlugValidation:
    ...
    def test_valid_slugs(self, client):
        ...
        for slug in valid_slugs:
            response = client.get(f'/notes/{slug}?key=test-admin-key')
...
class TestMarkdownRendering:
    ...
    def test_note_contains_markdown_scripts(self, client):
        """Note page includes markdown-it scripts."""
        client.get('/notes/render-test?key=test-admin-key')
        response = client.get('/notes/render-test')

        assert b'markdown-it' in response.data
        assert b'markdown-it-task-lists' in response.data
        assert b'dompurify' in response.data

    def test_note_json_data_embedded(self, client):
        """Note data is embedded in page for JS."""
        client.get('/notes/json-test?key=test-admin-key')
        response = client.get('/notes/json-test')
