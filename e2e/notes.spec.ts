...
test('rejects a stale save without discarding its draft', async ({ context, page }) => {
  ...
  const firstUpdateResponse = page.waitForResponse((response) =>
    response.request().method() === 'POST' &&
    response.url().endsWith(`/notes/${slug}`),
  );
  await page.getByRole('button', { name: 'Save' }).click();
  ...
  const conflictResponse = secondPage.waitForResponse((response) =>
    response.request().method() === 'POST' &&
    response.url().endsWith(`/notes/${slug}`),
  );
